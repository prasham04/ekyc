...
def extract_information(data_string, id_type, pan_format=None):
    updated_data_string = data_string.replace(".", "")
    words = [word.strip() for word in updated_data_string.split("|") if len(word.strip()) > 2]
    ...
    try:
        if id_type == "PAN":
            if pan_format == "New Format":
                # Process new PAN format

                id_number_index = words.index("GOVT OF INDIA") + 2
                extracted_info["ID"] = words[id_number_index]

                name_index = id_number_index + 1
                extracted_info["Name"] = words[name_index]
                
                father_index = name_index + 3
                extracted_info["Father's Name"] = words[father_index]
            ...
            # Process Aadhar card format
            for i, word in enumerate(words):
                if word.isdigit() and len(word) == 12:
                    extracted_info["ID"] = word
                elif "DOB" in word or "Year of Birth" in word:
                    try:
                        date_str = words[i + 1]
                        extracted_info["DOB"] = datetime.strptime(date_str, "%d/%m/%Y")
                    except ValueError:
                        continue
                elif i > 0 and not any(char.isdigit() for char in word):
                    if not extracted_info["Name"]:
                        extracted_info["Name"] = word
                    elif not extracted_info["Father's Name"]:
                        extracted_info["Father's Name"] = word
